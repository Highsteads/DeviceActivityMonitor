...
| Version | Date | Change |
|---------|------|--------|
| 1.11.1 | 07-Sep-2026 | **The settings dialog was stretched wider than its own window, so the help text beside each setting was cut off mid-sentence.** The short help that can be attached to a setting is drawn on a single line and never wraps, so the longest one decides how wide every row is — and the window cannot be widened past a fixed maximum. All three long ones have moved into ordinary description paragraphs, which do wrap. Two new checks fail the build if any help text or setting label grows long enough to do it again. No setting or behaviour changed. |
| 1.11.0 | 06-Sep-2026 | **The activity log moved out of the Indigo event log.** This plugin was writing about 836 lines a day into the shared event log - a third of everything in it - narrating every presence sensor going MOTION and CLEAR, with no way to turn it off. The narration now goes to the plugin's own log file, complete and timestamped as before, and a new tick box (with a menu item beside it) puts it back in the event log if you want it there. Faults are untouched and still appear in the event log: a deleted monitored device, a stale id in the config, an unreadable state, a group trigger that failed. Device and variable renames stay too, being rare and worth knowing about. 166 -> 181 tests. |
